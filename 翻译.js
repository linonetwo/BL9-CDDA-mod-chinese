...
const sourceModDirSuffix = '-monster-resilience-version';
const sourceModDirs = [
  `BL9-100%${sourceModDirSuffix}`,
 ...
 */
function writeToCNMod(foldersWithContent, sourceModDir) {
  for (const inspectDataWithContent of foldersWithContent) {
    const newFilePath = inspectDataWithContent.filePath.replace(
      `${sourceModDir}/`,
      `${sourceModDir}/`.replace(sourceModDirSuffix, '-CN')
    );
